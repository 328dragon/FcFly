 ...
 * 只有您的支持，匿名才能做得更好。  
===========================================================================*/
extern volatile u8 g_corner_red_count;
extern volatile u8 g_corner_blue_count;
extern volatile u8 g_corner_green_count;

extern volatile u8 fc_condition = 0;
extern volatile u8 turn_count = 0;
u8 send_buffer[50]; //发送数据缓存

_dt_st dt;

//////20260428 by qgq///////////////////////////////////////////
extern uint8_t _OpenMV_datatemp[50];
extern _Bool _WeHaveGotOpenMvOneFrame;
u16 tempcount=0;
...
void ANO_DT_Init(void)
{
	...
	//========外部触发
	//
	dt.fun[0x30].D_Addr = 0xff;  //GPS
	dt.fun[0x30].fre_ms = 200;	  //0 由外部触发
	...
	//20260614   qgq////////////////////////////////////
	dt.fun[0x35].D_Addr = 0xff;
	dt.fun[0x35].fre_ms = 100;	  //0 由外部触发
	dt.fun[0x35].time_cnt_ms = 0; //设置初始相位，单位1ms
...
{
	//
	UartSendLXIMU(dataToSend, length);  //在McuConfig.h中，被宏定义#define UartSendLXIMU 	DrvUart5SendBuf
}
...
static void ANO_DT_LX_Data_Receive_Anl(u8 *data, u8 len)
{
	...
	if ((check_sum1 != *(data + len - 2)) || (check_sum2 != *(data + len - 1))) //????sum????
		return;
	//再次判断帧头以及目标地址是否合法
...
static void Add_Send_Data(u8 frame_num, u8 *_cnt, u8 send_buffer[])
{
	...
	break;
	case 0x30: // GPS数据 + 飞控状态 + 拐角小球统计
{
    // 转发 OpenMV 原始23字节数据
    for (u8 i = 0; i < 23; i++)
    {
        send_buffer[(*_cnt)++] = _OpenMV_datatemp[i + 4];
    }
    ...
    // 飞控运动状态
    // ==================================================
    send_buffer[23] =fc_condition;
	send_buffer[15] =turn_count;	//拐角数
//	fc_condition;
}
break;
...
//		}
//		
//		/*send_buffer[(*_cnt)++]= 0x01;//qgq
//		send_buffer[(*_cnt)++]= 0x02;
//		send_buffer[(*_cnt)++]= 0x03;
//		send_buffer[(*_cnt)++]= 0x04;
//		send_buffer[(*_cnt)++]= 0x05;
//		send_buffer[(*_cnt)++]= 0x06;
//		send_buffer[(*_cnt)++]= 0x07;
//		send_buffer[(*_cnt)++]= 0x08;
//		send_buffer[(*_cnt)++]= 0x09;
//		send_buffer[(*_cnt)++]= 0x0A;//10
//		
//		//send_buffer[(*_cnt)++]= 0x0B;//10
//		//send_buffer[(*_cnt)++]= 0x0C;//10
//		//send_buffer[(*_cnt)++]= 0x0D;//13
//		//send_buffer[(*_cnt)++]= 0x0E;//14
//		//send_buffer[(*_cnt)++]= 0x0F;//15
//		//send_buffer[(*_cnt)++]= 0x10;//16
//		
//		tempcount=tempcount+1;
//		send_buffer[(*_cnt)++]= (u8)(tempcount & 0xFF);//11 // 低8位	
//		send_buffer[(*_cnt)++]= (u8)(tempcount >> 8); //12    // 高8位
//		send_buffer[(*_cnt)++]= 0x00;//13
//		send_buffer[(*_cnt)++]= 0x00;//14
//		send_buffer[(*_cnt)++]= 0x00;//15
//		send_buffer[(*_cnt)++]= 0x00;//16
//		*/
//	
//	
//		/*for (u8 i = 16; i < 23; i++)
...
break;
	...
	case 0x35: //qgq添加，上位机数据中0x35为未使用
	{
...
void ANO_LX_Data_Exchange_Task(float dT_s)
{
	...
	dt.fun[0x30].WTS = 1;	
	//=====检测是否触发发送
	if( _WeHaveGotOpenMvOneFrame == 1)
	{
		Check_To_Send(0x30);//GPS暂时使用
		_WeHaveGotOpenMvOneFrame = 0;
	}
	Check_To_Send(0x33);//速度传感器数据
	Check_To_Send(0x34);//测距传感器数据
	Check_To_Send(0x40);//遥控器数据
	Check_To_Send(0x41);//实时控制帧
	Check_To_Send(0xe0);//CMD命令帧
	Check_To_Send(0xe2);//PARA参数写入、参数读取返回
	Check_To_Send(0x0d);//电压电流数据
	/////////////////////////////////
	//dt.fun[0x35].WTS = 1;	
	//Check_To_Send(0x35);//借用，调试发现无法发送。
}
