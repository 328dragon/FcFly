...
uint8_t  g_openmv_line_state = 0;
uint8_t  g_openmv_next_direction = 0;
uint8_t  g_openmv_cross_flag=0;
int16_t  g_openmv_angle = 0;
int16_t  g_openmv_cross_x = 0;
int16_t  g_openmv_cross_y = 0;
	volatile u8 g_openmv_red_count = 0;
volatile u8 g_openmv_blue_count = 0;
volatile u8 g_openmv_green_count = 0;
volatile _Bool _WeHaveGotOpenMvOneFrame = 0;
...
///////20260428 by qgq///////////////////////////////////////
//OpenMV_DataAnl为OpenMV数据解析函数，可以通过本函数得到OpenMV模块输出的各项数据

static void OpenMV_DataAnl(uint8_t *data, uint8_t len)
{
    g_openmv_line_state     = data[4];
g_openmv_next_direction = data[5];
g_openmv_cross_flag=data[9];

g_openmv_angle =
    (int16_t)(((uint16_t)data[11] << 8) | data[10]);

//g_openmv_distance =
   // (int16_t)(((uint16_t)data[13] << 8) | data[12]);

g_openmv_cross_x =
    (int16_t)(((uint16_t)data[19] << 8) | data[18]);

g_openmv_cross_y =
    (int16_t)(((uint16_t)data[21] << 8) | data[20]);

g_openmv_red_count   = data[24];
g_openmv_blue_count  = data[25];
g_openmv_green_count = data[26];
    // 表示收到了一帧完整的OpenMV数据
    _WeHaveGotOpenMvOneFrame = 1;
}
///////////20260428by qgq////////////////////////////////////	
////初级数据解析函数，串口每接收到一字节OpenMV数据，调用本函数一次，函数参数就是串口收到的数据
//当本函数多次被调用，最终接收到完整的一帧数据后，会自动调用数据解析函数AnoOF_DataAnl
uint8_t _OpenMV_datatemp[50];
...
void OpenMV_GetOneByte(uint8_t data)
{
	...
	{
		rxstate = 1;
		_OpenMV_datatemp[0] = data;
	}
	else if (rxstate == 1 && (data == 0xAF))
	{
		rxstate = 2;
		_OpenMV_datatemp[1] = data;
	}
	else if (rxstate == 2 && data == 0x30)
	{
		rxstate = 3;
		_OpenMV_datatemp[2] = data;
	}
	else if (rxstate == 3 && data == 0x17)
	{
		rxstate = 4;
		_OpenMV_datatemp[3] = data;
		_data_len = data;
		_data_cnt = 0;
	}
	...
	{
		_data_len--;
		_OpenMV_datatemp[4 + _data_cnt++] = data;
		if (_data_len == 0)
			rxstate = 5;
	}
	else if (rxstate == 5)
	...
	else if (rxstate == 6)
	{
		...
		OpenMV_DataAnl(_OpenMV_datatemp, _data_cnt + 5); //
	}
	else
	{
		rxstate = 0;
...
void Usart3_IRQ(void)
{
    ...
    if (USART_GetITStatus(USART3, USART_IT_RXNE))
    {
        ...
        com_data = USART3->DR;
				...
				///20260402增加OpenMV通信，替换上一句/////////
				//OpenMV_Byte_Get( com_data );
				///////20260428 by qgq///////////////////////////////////////
				OpenMV_GetOneByte(com_data);
    }
    //发送（进入移位）中断
